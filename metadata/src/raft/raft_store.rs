...
impl MetadataStore for RaftMetadataStore {
    async fn get(&self, path: &str) -> Result<Option<FileMetadata>> {
        let key = format!("{}{}", std::str::from_utf8(crate::raft::utils::PATH_PREFIX).expect("PATH_PREFIX must be valid UTF-8"), path);
        let value = self.local_db.get_kv_value(key.as_bytes())
            .map_err(|e| common::Error::Internal(format!("Failed to list mount keys: {}", e)))?;
        if let Some(value) = value {
            let file_metadata: common::meta::FileMetadata = prost::Message::decode(value.as_slice())
                .map_err(|e| common::Error::Internal(format!("Failed to decode FileMetadata entry: {}", e)))?;
            let metadata = FileMetadata::try_from(file_metadata)
                .map_err(|e| common::Error::Internal(format!("Failed to convert FileMetadata: {}", e)))?;
    ...
    async fn put(&self, metadata: FileMetadata) -> Result<()> {
        tracing::info!("Putting FileMetadata for path {}: {:?}", metadata.path, metadata);
        self.raft_client.send_command(common::meta::MetaCmd {
            op: Some(common::meta::meta_cmd::Op::PutFileMeta(common::meta::PutFileMeta {
                full_path: metadata.path.clone(),
                file_metadata: Some(metadata.into()),
            })),
        }).await.map_err(|e| common::Error::Internal(format!("Failed to send put command: {}", e)))?;
        Ok(())
    }
    ...
    async fn save_mount(&self, mount_info: &MountInfo) -> Result<()> {
        ...
        }).await.map_err(|e| common::Error::Internal(format!("Failed to send mount command: {}", e)))?;
    ...
    async fn list_mounts(&self) -> Result<Vec<MountInfo>> {
        let keys =self.local_db.list_kv_keys(crate::raft::utils::MOUNT_PREFIX)
            .map_err(|e| common::Error::Internal(format!("Failed to list mount keys: {}", e)))?;
        let mut mounts = Vec::new();
        use prost::Message;
        for key in keys {
            if let Some(value) = self.local_db.get_kv_value(key.as_bytes())
                ...
                mounts.push(MountInfo {
                    ...
                    config: UfsConfig::try_from(mount.ufs_config

...
impl RocksStorage {
        ...
        if let Some(value) = self.get_kv_value(&key_bytes)? {
            let file_metadata: FileMetadata =
                prost::Message::decode(value.as_slice()).map_err(|e| {
                    Error::Internal(format!("Failed to decode FileMetadata entry: {}", e))
                })?;
    ...
    // Get BlockNode info for given node_id from KV CF
    pub async fn get_block_nodes(&self) -> Result<Vec<BlockNodeInfo>> {
        let mut nodes = Vec::new();
        let it = self.db.iterator_cf(
            &self.cf_kv,
            rocksdb::IteratorMode::From(BLOCK_NODE_PREFIX, rocksdb::Direction::Forward),
        );
        for kv in it {
            let (k, v) = kv?;
